...
export default function CrewDetail() {
  ...
  if (!member) return <div className="text-zinc-600 text-sm">Loading…</div>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        <Link to="/crew" className="text-sm text-zinc-600 hover:text-zinc-400">Crew</Link>
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-6">
        ...
        {member.skills && <p className="mt-2 text-sm text-zinc-500"><strong>Skills:</strong> {member.skills}</p>}
        {member.bio && <p className="mt-3 text-sm text-zinc-400 bg-zinc-950 rounded-lg p-3">{member.bio}</p>}
        {member.portal_code && (
          <div className="mt-3 flex items-center gap-2">
            <span className="text-xs text-zinc-500">Portal code:</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-zinc-900 rounded-xl border border-zinc-800">
          <h2 className="font-semibold text-white p-5 border-b border-zinc-900">Projects ({member.projects.length})</h2>
          <div className="divide-y divide-slate-100">
            {member.projects.length === 0 && <p className="p-5 text-sm text-zinc-600">Not assigned to any projects</p>}
            {member.projects.map(p => (
              <Link key={p.id} to={`/projects/${p.id}`} className="block p-4 hover:bg-zinc-950">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-white">{p.name}</p>
                    <p className="text-xs text-zinc-600">{p.role_on_project || '—'} {p.day_rate > 0 ? `• $${p.day_rate}/day` : ''}</p>
                  </div>
                  <div className="flex items-center gap-2">
        ...
        {/* Unavailability */}
        <div className="bg-zinc-900 rounded-xl border border-zinc-800">
          <h2 className="font-semibold text-white p-5 border-b border-zinc-900">Unavailable Dates</h2>
          <div className="p-5 space-y-3">
            <div className="flex gap-2">
              <input
                ...
                className="border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
              />
              <input
                type="text"
                ...
                className="flex-1 border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
              />
            ...
            <div className="space-y-1 max-h-48 overflow-y-auto">
              {member.unavailability.length === 0 && <p className="text-sm text-zinc-600">No dates blocked</p>}
