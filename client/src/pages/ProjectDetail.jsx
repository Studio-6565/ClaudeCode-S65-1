...
export default function ProjectDetail() {
  ...
  if (!project) return <div className="text-zinc-600 text-sm">Loading…</div>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        <Link to="/projects" className="text-sm text-zinc-600 hover:text-zinc-400">Projects</Link>
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-6">
        ...
        {project.description && <p className="mt-3 text-sm text-zinc-400">{project.description}</p>}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-5">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-zinc-500">Revenue</span><span className="font-medium text-green-600">${(f.revenue || 0).toLocaleString()}</span></div>
            <div className="border-t border-zinc-900 pt-2 space-y-1">
              <div className="flex justify-between text-zinc-500"><span>Crew cost</span><span>−${(f.crewCost || 0).toLocaleString()}</span></div>
              <div className="flex justify-between text-zinc-500"><span>Rental cost</span><span>−${(f.rentalCost || 0).toLocaleString()}</span></div>
              <div className="flex justify-between text-zinc-500"><span>Other cost</span><span>−${(f.otherCost || 0).toLocaleString()}</span></div>
            </div>
            <div className="border-t border-zinc-800 pt-2">
              <div className="flex justify-between font-medium"><span className="text-zinc-400">Total Cost</span><span className="text-red-500">−${(f.totalCost || 0).toLocaleString()}</span></div>
            </div>
            <div className="border-t border-zinc-800 pt-2">
              <div className="flex justify-between font-bold text-base">
                <span className="text-zinc-200">Profit</span>
        ...
        {/* Crew Roster */}
        <div className="lg:col-span-2 bg-zinc-900 rounded-xl border border-zinc-800">
          <div className="flex items-center justify-between p-5 border-b border-zinc-900">
            <h2 className="font-semibold text-white">Crew Roster</h2>
            {unassigned.length > 0 && (
          ...
          <div className="divide-y divide-slate-100">
            {project.crew.length === 0 && <p className="p-5 text-sm text-zinc-600">No crew assigned</p>}
            {project.crew.map(c => {
              ...
              return (
                <div key={c.crew_id} className="flex items-center justify-between px-5 py-3">
                  <div>
                    ...
                    <p className="text-xs text-zinc-500">{c.role_on_project || c.role || '—'} {c.day_rate > 0 ? `• $${c.day_rate}/day × ${c.days_worked}d` : ''}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    {cost > 0 && <span className="text-sm text-zinc-400">${cost.toLocaleString()}</span>}
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800">
        ...
        <div className="divide-y divide-slate-100">
          {project.deliverables.length === 0 && <p className="p-5 text-sm text-zinc-600">No deliverables</p>}
          {project.deliverables.map(d => (
            <div key={d.id} className="flex items-center justify-between px-5 py-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className={`text-sm font-medium ${d.status === 'Done' ? 'line-through text-zinc-600' : 'text-white'}`}>{d.title}</p>
                  {d.due_date && <p className="text-xs text-zinc-600">Due {d.due_date}</p>}
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Badge label={d.status} />
                <button onClick={() => { setDelivForm({ ...d }); setDelivModal(d); }} className="text-xs text-zinc-600 hover:text-red-600">Edit</button>
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800">
        <div className="flex items-center justify-between p-5 border-b border-zinc-900">
          <h2 className="font-semibold text-white">Events / Shoots</h2>
          <Link to="/schedule" className="text-xs text-red-600 hover:underline">Manage in Schedule</Link>
        </div>
        <div className="divide-y divide-slate-100">
          {project.events.length === 0 && <p className="p-5 text-sm text-zinc-600">No events linked</p>}
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800">
        ...
        <div className="divide-y divide-slate-100">
          {project.invoices.length === 0 && <p className="p-5 text-sm text-zinc-600">No invoices</p>}
          {project.invoices.map(inv => (
            <div key={inv.id} className="flex items-center justify-between px-5 py-3">
              <div>
                <p className="text-sm font-medium text-white">{inv.invoice_number || `INV-${inv.id}`}</p>
                {inv.due_date && <p className="text-xs text-zinc-600">Due {inv.due_date}</p>}
              </div>
              <div className="flex items-center gap-2">
      ...
      {crewModal && (
        <Modal title="Assign Crew" onClose={() => setCrewModal(false)}>
          <form onSubmit={assignCrew} className="space-y-3">
            ...
            <div>
              <label className="block text-xs font-medium text-zinc-400 mb-1">Role on Project</label>
              <input type="text" value={crewForm.role_on_project} onChange={e => setCrewForm(p => ({ ...p, role_on_project: e.target.value }))} className="w-full border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1">Day Rate ($)</label>
                <input type="number" min="0" step="0.01" value={crewForm.day_rate} onChange={e => setCrewForm(p => ({ ...p, day_rate: e.target.value }))} className="w-full border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
              </div>
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1">Days</label>
                <input type="number" min="0" step="0.5" value={crewForm.days_worked} onChange={e => setCrewForm(p => ({ ...p, days_worked: e.target.value }))} className="w-full border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
              </div>
            </div>
            <p className="text-xs text-zinc-600">An email will be sent to the crew member to confirm.</p>
        ...
        <Modal title={delivModal === 'add' ? 'Add Deliverable' : 'Edit Deliverable'} onClose={() => setDelivModal(null)}>
          <form onSubmit={saveDeliverable} className="space-y-3">
            <div>
              ...
              <input type="text" value={delivForm.title} onChange={e => setDelivForm(p => ({ ...p, title: e.target.value }))} required className="w-full border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1">Status</label>
                <select value={delivForm.status} onChange={e => setDelivForm(p => ({ ...p, status: e.target.value }))} className="w-full border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500">
              ...
              <div>
                ...
                <input type="date" value={delivForm.due_date} onChange={e => setDelivForm(p => ({ ...p, due_date: e.target.value }))} className="w-full border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
              </div>
            </div>
            <div>
              <label className="block text-xs font-medium text-zinc-400 mb-1">Notes</label>
              <textarea value={delivForm.notes} onChange={e => setDelivForm(p => ({ ...p, notes: e.target.value }))} rows={2} className="w-full border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <button type="button" onClick={() => setDelivModal(null)} className="px-4 py-2 text-sm text-zinc-400 hover:bg-zinc-800 rounded-lg">Cancel</button>
